...
func (c *Context) Morph(xmlBytes []byte) {
	...
	c.Putln("// Code generated by xgbgen from %s.xml. DO NOT EDIT.", c.protocol.Name)
	c.Putln("")
	c.Putln("// Package %s is the X client API for the %s extension.", c.protocol.PkgName(), c.protocol.ExtXName)
	...
	if c.protocol.isExt() {
		...
		c.Putln("// Init must be called before using the %s extension.", xname)
		c.Putln("func Init(c *xgb.Conn) error {")
		c.Putln("reply, err := xproto.QueryExtension(c, %d, \"%s\").Reply()", len(xname), xname)
		c.Putln("switch {")
		...
		c.Putln("for evNum, fun := range xgb.NewExtEventFuncs[\"%s\"] {", xname)
		c.Putln("xgb.NewEventFuncs[int(reply.FirstEvent) + evNum] = fun")
		c.Putln("}")
		c.Putln("for errNum, fun := range xgb.NewExtErrorFuncs[\"%s\"] {", xname)
		...
		c.Putln("func init() {")
		c.Putln("xgb.NewExtEventFuncs[\"%s\"] = make(map[int]xgb.NewEventFun)", xname)
		c.Putln("xgb.NewExtErrorFuncs[\"%s\"] = make(map[int]xgb.NewErrorFun)", xname)
		c.Putln("}")
		c.Putln("")
	} else {
